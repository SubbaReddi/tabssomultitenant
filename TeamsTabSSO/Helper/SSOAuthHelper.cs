...
namespace TeamsTabSSO.Helper
{
    ...
    public class SSOAuthHelper
    {
        ...
        public static async Task<string> GetAccessTokenOnBehalfUserAsync(IConfiguration configuration, IHttpClientFactory httpClientFactory, IHttpContextAccessor httpContextAccessor,string tenantidvalue)
        {
            ...
            var body = $"assertion={idToken}&requested_token_use=on_behalf_of&grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer&client_id={configuration[ClientIdConfigurationSettingsKey]}@{tenantidvalue}&client_secret={configuration[AppsecretConfigurationSettingsKey]}&prompt=consent&scope=https://graph.microsoft.com/User.Read offline_access email openid profile";
            try
            {
                var client = httpClientFactory.CreateClient("WebClient");
                string responseBody;
                using (var request = new HttpRequestMessage(HttpMethod.Post, configuration[AzureInstanceConfigurationSettingsKey] + tenantidvalue + configuration[AzureAuthUrlConfigurationSettingsKey]))
                {
